...
from cabot.cabotapp.alert import AlertPluginUserData, send_alert, send_alert_update
from cabot.cabotapp.calendar import get_events
from cabot.cabotapp.graphite import parse_metric
from cabot.cabotapp.tasks import update_instance, update_service
...
class CheckGroupMixin(models.Model):
    ...
    alerts = models.ManyToManyField(
        "AlertPlugin", blank=True, help_text="Alerts channels through which you wish to be notified"
    )
        ...
        if self.overall_status != self.PASSING_STATUS:
            ...
            if self.overall_status == self.WARNING_STATUS:
                if (
                    ...
                    and (timezone.now() - timedelta(minutes=settings.NOTIFICATION_INTERVAL))
                    < self.last_alert_sent
                ):
                    return
...
class StatusCheck(PolymorphicModel):
    ...
    def run(self):
        ...
        try:
            ...
            result.error = f"Error in performing check: {e}"
            result.succeeded = False
        finish = timezone.now()
        result.time = start
...
def minimize_targets(targets):
    ...
    for i, nodes in enumerate(zip(*split)):
    ...
    for i, nodes in enumerate(reversed(list(zip(*split)))):
        if any(node != nodes[0] for node in nodes):
...
class GraphiteStatusCheck(StatusCheck):
    ...
    def format_error_message(self, failures, actual_hosts, hosts_by_target):
        if actual_hosts < self.expected_num_hosts:
            return "Hosts missing | %d/%d hosts" % (actual_hosts, self.expected_num_hosts)
        elif actual_hosts > 1:
            threshold = float(self.value)
            failures_by_host = [
                "%s: %s %s %0.1f" % (hosts_by_target[target], value, self.check_type, threshold)
    ...
    def _run(self):
        ...
        if last_result:
            ...
            time_to_check = max(
                self.frequency, ((timezone.now() - last_result_started).total_seconds() / 60) + 1
            )
        else:
...
class HttpStatusCheck(StatusCheck):
    ...
    @classmethod
    def _check_content_pattern(self, text_match, content: str | bytes) -> bool:
        if isinstance(content, bytes):
            content = content.decode("UTF-8")
        return bool(re.search(text_match, content))
    ...
    def _run(self):
        ...
        try:
            ...
            result.error = f"Request error occurred: {e}"
            result.succeeded = False
        else:
            if self.status_code and resp.status_code != int(self.status_code):
                ...
                if not self._check_content_pattern(self.text_match, resp.text):
                    result.error = (
                        "Failed to find match regex /%s/ in response body" % self.text_match
                    )
                    result.raw_data = resp.text
                    result.succeeded = False
                else:
...
class StatusCheckResult(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=("status_check", "time_complete")),
            models.Index(fields=("status_check", "id")),
        ]
...
class Shift(models.Model):
    ...
    def __unicode__(self):
        ...
        if self.deleted:
            deleted = " (deleted)"
        return f"{self.user}: {self.start} to {self.end}{deleted}"
