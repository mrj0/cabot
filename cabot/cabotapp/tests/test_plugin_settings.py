...
class PluginSettingsTest(TestCase):
    ...
    def test_plugin_settings(self):
        ...
        resp = self.client.get(
            resolve_url("plugin-settings", plugin_name=plugin.title), follow=True
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_plugin_disable(self):
        ...
        resp = self.client.post(
            resolve_url("plugin-settings", plugin_name=plugin.title),
            {"enabled": False},
            follow=True,
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_plugin_alert_test(self, fake_send_alert):
        ...
        resp = self.client.post(
            resolve_url("alert-test-plugin"),
            {"alert_plugin": plugin.id, "old_status": "PASSING", "new_status": "ERROR"},
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_global_alert_test(self, fake_send_alert):
        ...
        resp = self.client.post(
            resolve_url("alert-test"),
            {"service": service.id, "old_status": "PASSING", "new_status": "ERROR"},
        )
        self.assertEqual(resp.status_code, 200)
        self.assertIn("ok", resp.content)
        fake_send_alert.assert_called()
