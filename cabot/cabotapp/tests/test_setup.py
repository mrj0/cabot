...
class SetupTest(TestCase):
    ...
    def test_initial_setup_post(self):
        resp = self.client.post(
            resolve_url("first_time_setup"), data={"username": "", "password": "pass"}
        )
        self.assertEqual(resp.status_code, 400)

        resp = self.client.post(
            resolve_url("first_time_setup"), data={"username": "test", "password": ""}
        )
        self.assertEqual(resp.status_code, 400)
        self.assertFalse(get_user_model().objects.exists())

        resp = self.client.post(
            resolve_url("first_time_setup"), data={"username": "test", "password": "pass"}
        )
        self.assertEqual(resp.status_code, 302)
    ...
    def test_initial_setup_post_with_email(self):
        resp = self.client.post(
            ...
            data={"username": "test", "email": "fail", "password": "pass"},
        )
        self.assertEqual(resp.status_code, 400)
        self.assertFalse(get_user_model().objects.exists())
        ...
        resp = self.client.post(
            resolve_url("first_time_setup"),
            data={"username": "test", "email": "real@email.com", "password": "pass"},
        )
        self.assertEqual(resp.status_code, 302)
        self.assertTrue(get_user_model().objects.exists())
