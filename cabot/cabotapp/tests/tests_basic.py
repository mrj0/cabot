...
class LocalTestCase(APITestCase):
    ...
    def create_dummy_data(self):
        ...
        os.environ.setdefault("JENKINS_API", "http://jenkins:8080")
        create_default_jenkins_config()
...
def fake_http_200_response(*args, **kwargs):
    ...
    resp.text = resp.content.encode("utf-8")
    resp.status_code = 200
    return resp
...
def throws_timeout(*args, **kwargs):
    raise requests.RequestException("фиктивная ошибка innit")
...
class TestCheckRun(LocalTestCase):
    ...
    def test_graphite_run(self):
        ...
        self.graphite_check.utcnow = (
            1387818601  # see graphite_response.json for this magic timestamp
        )
        ...
        self.assertEqual(result.error, "PROD: 9.16092 > 9.0")

    @patch("cabot.cabotapp.graphite.requests.get")
    def test_graphite_series_run_exception(self, fake_graphite_series_response):
        fake_graphite_series_response.side_effect = requests.exceptions.RequestException(
            "some error"
        )
    ...
    def test_timeout_handling_in_jenkins(self):
        ...
        self.assertIn(
            "Error fetching from Jenkins - фиктивная ошибка", self.jenkins_check.last_result().error
        )
    ...
    def test_http_run(self):
        ...
        # Unicode
        self.http_check.text_match = "как закалялась сталь"
        self.http_check.save()
        self.http_check.run()
        self.assertFalse(self.http_check.last_result().succeeded)
        self.assertEqual(self.http_check.calculated_status, Service.CALCULATED_FAILING_STATUS)
        self.assertIn("Failed to find match regex", self.http_check.last_result().error)
    ...
    def test_timeout_handling_in_http(self):
        ...
        self.assertFalse(self.http_check.last_result().succeeded)
        self.assertIn(
            "Request error occurred: фиктивная ошибка innit", self.http_check.last_result().error
        )
...
class TestWebInterface(LocalTestCase):
    ...
    def test_set_recovery_instructions(self):
        ...
        resp = self.client.get(
            reverse("update-service", kwargs={"pk": self.service.id}), follow=True
        )
...
class TestAPI(LocalTestCase):
    def setUp(self):
        ...
        self.post_data = {
            ...
            "graphitestatuscheck": [
                {
                    "name": "posted graphite check",
                    "active": True,
                    "importance": "CRITICAL",
                    "frequency": 5,
                    "debounce": 0,
                    "metric": "stats.fakeval2",
                    "check_type": "<",
                    "value": "2",
                    "expected_num_hosts": 0,
                    "allowed_num_failures": 0,
            ...
            "httpstatuscheck": [
                {
                    "name": "posted http check",
                    "active": True,
                    "importance": "ERROR",
                    "frequency": 5,
                    "debounce": 0,
                    "endpoint": "http://arachnys.com/post_tests",
                    "username": None,
                    "password": None,
                    "text_match": "text",
                    "status_code": "201",
...
class TestHttpStatusCheck(TestCase):
    PATTERN_DATASET = [
        {"pattern": "буюЯзйЪ", "content": "буюЯзйЪ", "result": True},
        {"pattern": "юЯз", "content": "буюЯзйЪ", "result": True},
        {"pattern": "юЯз1", "content": "буюЯзйЪ", "result": False},
        {"pattern": "sti", "content": "testing", "result": True},
        {"pattern": "testing", "content": "testing", "result": True},
        {"pattern": "test", "content": "testing", "result": True},
        {"pattern": "test", "content": "testing", "result": True},
        {"pattern": "ting", "content": "testing", "result": True},
        {"pattern": "日出處天子", "content": "日出處天子", "result": True},
        {"pattern": "出處", "content": "日出處天子", "result": True},
        {"pattern": "出處", "content": "日出處天子", "result": True},
        {"pattern": "日出天子", "content": "日出處天子", "result": False},
        {"pattern": "юЯз", "content": "日出處天子", "result": False},
        {"pattern": "юЯз", "content": "日出處天子", "result": False},
    ]
