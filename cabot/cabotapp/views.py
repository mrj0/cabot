...
class LoginRequiredMixin(object):
    ...
    def dispatch(self, *args, **kwargs):
        return super(LoginRequiredMixin, self).dispatch(*args, **kwargs)  # noqa
...
class InstanceForm(SymmetricalForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["users_to_notify"].queryset = User.objects.filter(is_active=True).order_by(
...
class StatusCheckReportForm(forms.Form):
    ...
    def get_report(self):
        ...
        for check in checks:
            ...
            pairs = zip_longest(([iter(times)] * 2), fillvalue=None)
...
class StatusCheckDetailView(LoginRequiredMixin, CommonDetailView):
    ...
    def render_to_response(self, context, *args, **kwargs):
        if context is None:
            context = {}
        checkresult_list = self.object.statuscheckresult_set.order_by("-time_complete").all()
...
class UserProfileUpdateView(LoginRequiredMixin, View):
    ...
    def get(self, *args, **kwargs):
        return HttpResponseRedirect(
            reverse("update-alert-user-data", args=(self.kwargs["pk"], "General"))
        )
...
class InstanceCreateView(LoginRequiredMixin, CommonCreateView):
    ...
    def form_valid(self, form):
        ...
        if (
            self.object.status_checks.filter(polymorphic_ctype__model="icmpstatuscheck").count()
            == 0
        ):
...
class ShiftListView(LoginRequiredMixin, CommonListView):
    ...
    def get_queryset(self):
        return Shift.objects.filter(end__gt=datetime.now(UTC), deleted=False).order_by("start")
...
def checks_run_recently(request):
    ...
    """
    ten_mins = datetime.now(UTC) - timedelta(minutes=10)
